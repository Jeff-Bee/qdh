...
namespace Laplace.Framework.Helper
{
    ...
    public class CCPRestSDK
    {
        ...
        {
            this._mainAccount = accountSid;
            this._mainToken = accountToken;
        }
        ...
        {
            this._subAccount = subAccountSid;
            this._subToken = subAccountToken;
            this._voipAccount = voipAccount;
            this._voipPwd = voipPassword;
        }
        ...
        public Dictionary<string, object> QueryAccountInfo()
        {
            ...
            try
            {
                ...
                string uriStr = string.Format("https://{0}:{1}/{2}/Accounts/{3}/AccountInfo?sig={4}", _restAddress, _restPort, softVer, _mainAccount, sigstr);
        ...
        public Dictionary<string, object> CreateSubAccount(string friendlyName)
        {
            ...
            try
            {
                ...
                // 构建URL内容
                string sigstr = MD5Encrypt(_mainAccount + _mainToken + date);
                string uriStr = string.Format("https://{0}:{1}/{2}/Accounts/{3}/SubAccounts?sig={4}", _restAddress, _restPort, softVer, _mainAccount, sigstr);
                Uri address = new Uri(uriStr);
        ...
        public Dictionary<string, object> GetSubAccounts(uint startNo, uint offset)
        {
            ...
            try
            {
                ...
                // 构建URL内容
                string sigstr = MD5Encrypt(_mainAccount + _mainToken + date);
                string uriStr = string.Format("https://{0}:{1}/{2}/Accounts/{3}/GetSubAccounts?sig={4}", _restAddress, _restPort, softVer, _mainAccount, sigstr);
                Uri address = new Uri(uriStr);
        ...
        public Dictionary<string, object> QuerySubAccount(string friendlyName)
        {
            ...
            try
            {
                ...
                // 构建URL内容
                string sigstr = MD5Encrypt(_mainAccount + _mainToken + date);
                string uriStr = string.Format("https://{0}:{1}/{2}/Accounts/{3}/QuerySubAccountByName?sig={4}", _restAddress, _restPort, softVer, _mainAccount, sigstr);
                Uri address = new Uri(uriStr);
        ...
        public Dictionary<string, object> SendSMS(string to, string body)
        {
            ...
            try
            {
                ...
                // 构建URL内容
                string sigstr = MD5Encrypt(_mainAccount + _mainToken + date);
                string uriStr = string.Format("https://{0}:{1}/{2}/Accounts/{3}/SMS/Messages?sig={4}", _restAddress, _restPort, softVer, _mainAccount, sigstr);
                Uri address = new Uri(uriStr);
        ...
        public Dictionary<string, object> SendTemplateSMS(string to, string templateId, string[] data)
        {
            ...
            try
            {
                ...
                // 构建URL内容
                string sigstr = MD5Encrypt(_mainAccount + _mainToken + date);
                string uriStr = string.Format("{0}:{1}/{2}/Accounts/{3}/SMS/TemplateSMS?sig={4}", _restAddress, _restPort, softVer, _mainAccount, sigstr);
                Uri address = new Uri(uriStr);
        ...
        public Dictionary<string, object> CallBack(string from, string to, string customerSerNum, string fromSerNum, string promptTone)
        {
            ...
            try
            {
                ...
                // 构建URL内容
                string sigstr = MD5Encrypt(_subAccount + _subToken + date);
                string uriStr = string.Format("https://{0}:{1}/{2}/SubAccounts/{3}/Calls/Callback?sig={4}", _restAddress, _restPort, softVer, _subAccount, sigstr);
                Uri address = new Uri(uriStr);
                ...
                Encoding myEncoding = Encoding.GetEncoding("utf-8");
                byte[] myByte = myEncoding.GetBytes(_subAccount + ":" + date);
                string authStr = Convert.ToBase64String(myByte);
                request.Headers.Add("Authorization", authStr);
        ...
        public Dictionary<string, object> LandingCall(string to, string mediaName, string mediaTxt, string displayNum, string playTimes, int type, string respUrl)
        {
            ...
            try
            {
                ...
                // 构建URL内容
                string sigstr = MD5Encrypt(_mainAccount + _mainToken + date);
                string uriStr = string.Format("https://{0}:{1}/{2}/Accounts/{3}/Calls/LandingCalls?sig={4}", _restAddress, _restPort, softVer, _mainAccount, sigstr);
                Uri address = new Uri(uriStr);
        ...
        public Dictionary<string, object> VoiceVerify(string to, string verifyCode, string displayNum, string playTimes, string respUrl)
        {
            ...
            try
            {
                ...
                // 构建URL内容
                string sigstr = MD5Encrypt(_mainAccount + _mainToken + date);
                string uriStr = string.Format("https://{0}:{1}/{2}/Accounts/{3}/Calls/VoiceVerify?sig={4}", _restAddress, _restPort, softVer, _mainAccount, sigstr);
                Uri address = new Uri(uriStr);
        ...
        public Dictionary<string, object> IvrDial(string number, string userdata, string record)
        {
            ...
            try
            {
                ...
                // 构建URL内容
                string sigstr = MD5Encrypt(_mainAccount + _mainToken + date);
                string uriStr = string.Format("https://{0}:{1}/{2}/Accounts/{3}/ivr/dial?sig={4}", _restAddress, _restPort, softVer, _mainAccount, sigstr);
                Uri address = new Uri(uriStr);
        ...
        public Dictionary<string, object> BillRecords(string date, string keywords)
        {
            ...
            try
            {
                ...
                string uriStr = string.Format("https://{0}:{1}/{2}/Accounts/{3}/BillRecords?sig={4}", _restAddress, _restPort, softVer, _mainAccount, sigstr);
                Uri address = new Uri(uriStr);
